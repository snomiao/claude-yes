...
async function initSchema(db: Kysely<DatabaseSchema>): Promise<void> {
  try {
    ...
    await db.schema
      ...
      .addColumn("started_at", "integer", (col) => col.notNull().defaultTo(sql`(unixepoch())`))
    ...
    await db.schema.createIndex("idx_runs_pid").ifNotExists().on("runs").column("pid").execute();
  } catch (err: unknown) {
 ...
 */
export async function upsertProject(params: { dir: string; cmd: string; prompt?: string | null }) {
...
export async function getProjectByDir(dir: string) {
  await ensureInitialized();
  const db = getDb();
  return await db.selectFrom("projects").selectAll().where("dir", "=", dir).executeTakeFirst();
}
...
export async function getAllProjects() {
  await ensureInitialized();
  const db = getDb();
  return await db.selectFrom("projects").selectAll().orderBy("last_run_at", "desc").execute();
}
 ...
 * Create a new run
 */
export async function createRun(params: { pid: number; project_id: number; args?: string[] }) {
  await ensureInitialized();
  const db = getDb();
...
export async function getRunById(id: number) {
  await ensureInitialized();
  const db = getDb();
  const result = await db.selectFrom("runs").selectAll().where("id", "=", id).executeTakeFirst();
...
export async function getRunningRuns() {
  await ensureInitialized();
  const db = getDb();
  const results = await db.selectFrom("runs").selectAll().where("status", "=", "running").execute();
...
export async function getAllRuns() {
  await ensureInitialized();
  const db = getDb();
  const results = await db.selectFrom("runs").selectAll().orderBy("started_at", "desc").execute();
...
export async function deleteProjectRuns(projectId: number) {
  await ensureInitialized();
  const db = getDb();
  return await db.deleteFrom("runs").where("project_id", "=", projectId).execute();
}
